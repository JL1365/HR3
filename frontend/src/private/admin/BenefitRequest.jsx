...
function BenefitRequest() {
    const { allBenefitRequests, fetchAllBenefitRequest, loading, error, clearError,updateBenefitRequestStatus } = useBenefitRequestStore();
    ...
    const getBenefitName = (request) => {
        if (request.compensationBenefitId && typeof request.compensationBenefitId === 'object') {
            return request.compensationBenefitId.name || request.compensationBenefitId.benefitName || "N/A";
        }
                ...
                <div className="overflow-x-auto rounded-lg shadow">
                    ...
                    >
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {allBenefitRequests.map((request) => (
                                <motion.tr 
                                    key={request._id}
                                    variants={rowVariants}
                                    whileHover={{ backgroundColor: "#f9fafb" }}
                                >
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{request._id}</td>
                                        ...
                                        <div className="flex space-x-2">
                                            {request.uploadDocs?.frontId && (
                                                <div className="relative">
                                                    <motion.div
                                                        className="relative"
                                                        onHoverStart={() => setImageTooltip(`${request._id}-front`)}
                                                        onHoverEnd={() => setImageTooltip(null)}
                                                    >
                                                        <a href={request.uploadDocs.frontId} target="_blank" rel="noopener noreferrer">
                                                            <motion.img 
                                            ...
                                            {request.uploadDocs?.backId && (
                                                <div className="relative">
                                                    <motion.div
                                                        className="relative"
                                                        onHoverStart={() => setImageTooltip(`${request._id}-back`)}
                                                        onHoverEnd={() => setImageTooltip(null)}
                                                    >
                                                        <a href={request.uploadDocs.backId} target="_blank" rel="noopener noreferrer">
                                                            <motion.img 
